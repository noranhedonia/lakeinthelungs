...
LAKE_HOT_FN
LAKEAPI lake_work_chain LAKECALL 
lake_acquire_chain_(usize initial_value);

#define lake_acquire_chain() lake_acquire_chain_(1)

/** Decrement a chain's value. Should be used only for externally acquired chains.
 *  @return The last value of the chain. */
LAKE_FORCE_INLINE usize 
lake_decrement_chain(lake_work_chain chain)
{ return lake_atomic_sub_explicit(chain, 1lu, lake_memory_model_release); }

/** Release a chain acquired externally to a work submission. */
LAKE_FORCE_INLINE void
lake_release_chain(lake_work_chain chain)
...
LAKE_THREAD_SAFETY_ACQUIRE_SHARED(3) LAKE_NONNULL(2) LAKE_HOT_FN 
...
LAKE_THREAD_SAFETY_RELEASE_SHARED(1) LAKE_HOT_FN 
LAKEAPI void LAKECALL 
lake_yield(
