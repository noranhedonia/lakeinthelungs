    ...
    char const *type)
{
    ...
    if (!(v = __lake_malloc(stride * n, align))) return LAKE_ERROR_OUT_OF_HOST_MEMORY;
    ...
    if (deq->len) {
        ...
        if (part2) lake_memcpy(lake_elem(v, stride, part1), deq->v, stride * part2);
    }
    if (deq->cap) __lake_free(deq->v);
    deq->v = v;
    deq->head = 0;
    deq->tail = deq->len;
    return LAKE_SUCCESS;
