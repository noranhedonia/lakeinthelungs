...
static PFN_vkVoidFunction get_vk_instance_proc_address(struct moon_impl const *moon, char const *procname)
{
...
static PFN_vkVoidFunction get_vk_device_proc_address(struct moon_device_impl const *device, char const *procname)
{
    PFN_vkVoidFunction address = device->header.moon.impl->vkGetDeviceProcAddr(device->vk_device, procname);
    if (address == nullptr)
        lake_warn("Can't find Vulkan device procedure `%s`.", procname);
    return address;
}

static bool load_vk_instance_symbols(struct moon_impl *moon, u32 extension_bits)
{
    lake_dbg_assert(moon != nullptr && moon->vkGetInstanceProcAddr, LAKE_ERROR_MEMORY_MAP_FAILED, nullptr);
...
static bool load_vk_device_symbols(struct moon_device_impl *device, u64 extension_bits)
{
    lake_dbg_assert(device != nullptr && device->header.moon.impl != nullptr 
        && device->header.moon.impl->vkGetDeviceProcAddr, LAKE_ERROR_MEMORY_MAP_FAILED, nullptr);
    ...
    if (device->header.moon.impl->vk_debug_messenger != VK_NULL_HANDLE) {
...
static constexpr usize g_required_feature__fragment_stores_and_atomics_features[] = {
    ...
    lake_offsetof(struct physical_device_features, features2.features.vertexPipelineStoresAndAtomics),
};
...
static void query_physical_device_features(struct physical_device_features *features, u64 extension_bits)
{
    ...
    features->maintenance6.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MAINTENANCE_6_FEATURES;
    features->maintenance6.pNext = chain;
    chain = &features->maintenance6;

    features->maintenance5.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MAINTENANCE_5_FEATURES;
    features->maintenance5.pNext = chain;
    chain = &features->maintenance5;
    
    features->maintenance4.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MAINTENANCE_4_FEATURES;
...
static FN_LAKE_WORK(query_physical_device, struct query_physical_device_work *work)
{
    ...
    struct moon_impl const *moon = work->moon;
    ...
    for (u32 i = 0; i < queue_family_count; i++) {
        ...
        if (!(found_queue_families & (1u << moon_queue_type_main)) && flags & VK_QUEUE_GRAPHICS_BIT) {
            ...
            unique_type = moon_queue_type_main;
...
/* TODO sparse binding & video features */
#if 0
...
/* TODO need to refactor the physical device unique family indices first
        if ((type == moon_queue_type_sparse_binding && !(pd->details.implicit_features & moon_implicit_feature_sparse_binding)) 
            || ((type == moon_queue_type_video_decode) && !(explicit_features & moon_explicit_feature_video_decode_queue))
...
FN_MOON_CONNECT_TO_DISPLAY(vulkan)
{
    ...
    hadal_interface impl = { .impl = hadal };
...
static struct moon_impl *g_moon = nullptr;

static FN_LAKE_WORK(_moon_vulkan_zero_refcnt, struct moon_impl *moon)
{
#ifndef LAKE_NDEBUG
    ...
    lake_assert(refcnt <= 0, LAKE_HANDLE_STILL_REFERENCED, "Interface %s reference count is %d.", moon->interface.header.name.str, refcnt);

    if (moon->vk_debug_messenger)
...
LAKEAPI FN_LAKE_INTERFACE_IMPL(moon, vulkan, lake_framework)
{
    ...
    struct moon_impl *moon = __lake_malloc_t(struct moon_impl);
    lake_zerop(moon);
    g_moon = moon;
    ...
    moon->interface.read_buffer_assembly = _moon_vulkan_read_buffer_assembly;
    moon->interface.read_texture_assembly = _moon_vulkan_read_texture_assembly;
    moon->interface.read_texture_view_assembly = _moon_vulkan_read_texture_view_assembly;
    moon->interface.read_sampler_assembly = _moon_vulkan_read_sampler_assembly;
    moon->interface.read_tlas_assembly = _moon_vulkan_read_tlas_assembly;
    moon->interface.read_blas_assembly = _moon_vulkan_read_blas_assembly;
    ...
    moon->interface.work_graph_pipeline_node_index = _moon_vulkan_work_graph_pipeline_node_index;
    moon->interface.work_graph_pipeline_scratch_size = _moon_vulkan_work_graph_pipeline_scratch_size;
    ...
    moon->interface.cmd_pipeline_barrier = _moon_vulkan_cmd_pipeline_barrier;
    moon->interface.cmd_signal_event = _moon_vulkan_cmd_signal_event;
    moon->interface.cmd_reset_event = _moon_vulkan_cmd_reset_event;
    moon->interface.cmd_wait_events = _moon_vulkan_cmd_wait_events;
    ...
    moon->interface.cmd_dispatch_graph_scratch_memory = _moon_vulkan_cmd_dispatch_graph_scratch_memory;
